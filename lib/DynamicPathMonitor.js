...
DynamicPathMonitor.prototype = {
	_add: function (snap) {
		var name = snap.key.toLowerCase();
		var pathProps = snap.val();
		pathProps.index = pathProps.index || name;
		pathProps.type = pathProps.type || pathProps.connectorId.toLowerCase();
		pathProps.path = pathProps.path || pathProps.endpoint;
		if (isValidPath(pathProps)) {
			...
			var er = {
				index: pathProps.index,
				path: pathProps.path,
				type: pathProps.type
			};
			console.error('Invalid dynamic path fetched from db. Most be an object with index, type, and path attributes.', name, er);
		}
	},
...
function isValidPath(props) {
	return props && typeof (props) === 'object' && ((props.index && props.path && props.type) || (props.key && props.endpoint && props.connectorId));
}
