...
function PathMonitor(esc, fbAuthTok, path) {
   this.ref = fbutil.fbRef(fbAuthTok, path.path);
...
exports.process = function(esc, fbAuthTok, paths, dynamicPathUrl) {
   paths && paths.forEach(function(pathProps) {
      new PathMonitor(esc, fbAuthTok, pathProps);
   });
   if (dynamicPathUrl) {
