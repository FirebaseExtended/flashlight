...
function AbstractPathMonitor(esc, root, path) {
  this.ref = root.child(path.path);
  ...
  this.sorted = ( path.sorted===true ); // If path has meaningful priority set, paginate to minimize the cache
            ...
            snaps.filter(function(s) { return self.filter(s.val()); })
              .map(function(snap) {
                ...
                return self.parse( snap.val() ).then( function(value) {
                  return { id: snap.key(), value: value };
                })
              })
        ...
        if (typeof(value)==='object') {
          ...
          if (isPseudoArray) {
            ...
            Object.keys(value).forEach( function(k) {
              value[k] = simpleSanitize(value[k]);
            });
