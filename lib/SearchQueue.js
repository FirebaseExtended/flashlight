...
function SearchQueue(esc, reqRef, resRef, cleanupInterval) {
	...
	console.log('Queue started, IN: "%s", OUT: "%s"'.green, fbutil.pathName(this.inRef), fbutil.pathName(this.outRef));
	setTimeout(function () {
...
SearchQueue.prototype = {
	_process: function (snap) {
		...
		console.log('processing query request'.green, key);
	...
	_replyError: function (key, err) {
		this._send(key, {
			total: 0,
			error: fbutil.unwrapError(err),
			timestamp: Date.now()
		});
	},

	_send: function (key, data, reqData) {
		var outputEndpoint;

		if (reqData && reqData.outputEndpoint) {
			outputEndpoint = fbutil.fbRef(reqData.outputEndpoint);
		} else {
			outputEndpoint = this.outRef.child(key);
		}
	...
	_nextInterval: function () {
		...
		console.log('Next cleanup in %d %s'.green, Math.round(this.cleanupInterval / (interval === 'seconds' ? 1000 : 60000)), interval);
...
exports.init = function (esc, reqPath, resPath, matchWholeWords, cleanupInterval) {
	console.log("Initializing firebase search queue");
