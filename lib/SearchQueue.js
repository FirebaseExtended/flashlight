...
SearchQueue.prototype = {
  ...
  _housekeeping: function() {
    ...
    this.outRef.endAt(new Date().valueOf() - self.cleanupInterval).set(null).then(function(err) {
      if (err) {
        console.log("Mainenance failed: " + err);
      }
      else {
        console.log("Performed maintenance, nuking all stale responses older than " + self.cleanupInterval/1000 + " seconds.");
      }
